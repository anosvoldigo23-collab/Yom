...
module.exports = {
  ...
  onStart: async function ({ api, event, args }) {
    ...
    try {
      ...
      const sendingMsg = await api.sendMessage("📝 Création du post en cours...", threadID);
...
║ ✅ Publication réussie !
╠══════════════════╣
║ 🔗 Lien : ${url || "Non retourné"}
║ 📄 Texte : ${postMessage || "Aucun texte fourni"}
║ 📎 Médias : ${files.length} fichier(s)
╚══════════════════╝
          `;
          api.sendMessage(framedMessage, threadID, messageID);
        })
        .catch((error) => {
